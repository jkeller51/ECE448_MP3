...
class Network(object):
    ...
    def _loss(self, f, y):
        ...
        if (len(f.shape) == 2):
            ...
            for i in range(len(y)):
                onef = np.zeros(np.size(f,1))
                onef[int(y[i])] = 1
                L +=  np.log(f[i,int(y[i])])
                dF[i] = onef-f[i,:]
            L *= -1/(len(f))
            dF *= -1/(len(f))
        return L, dF
    ...
    def save_weights(self):
        ...
        """
        # Structure
        filename = "./dnn_weights/structure.txt"
        os.makedirs(os.path.dirname(filename), exist_ok=True)
        with open(filename, 'w') as myfile:
            myfile.write(' '.join(str(x) for x in self.hidden_layers))
        
        # Parameters
        filename = "./dnn_weights/parameters.txt"
        os.makedirs(os.path.dirname(filename), exist_ok=True)
        with open(filename, 'w') as myfile:
            myfile.write('bias,{0}\n'.format(self.bias))
        ...
        for i in range(len(self.W)):
            filename = "./dnn_weights/weight_{0}.txt".format(i)
    ...
    def load_weights(self):
        ...
        """
        # Structure
        structure = np.genfromtxt('./dnn_weights/structure.txt', dtype=np.int16)
        ...
        for i in range(self.layers+1):
            myfile = './dnn_weights/weight_{0}.txt'.format(i)
    ...
    def train(self, X, y):
        ...
        plt.xlabel('Epoch')
        plt.ylabel('Loss')
        plt.savefig('dnn_learning_curve.png', dpi=1600)
